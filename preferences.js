...
function resolveSiteThemePreference() {
  try {
    ...
    if (username) {
      const preferences = getUserPreferences(username);
      if (preferences && preferences.theme === 'light') return 'light';
      if (preferences && preferences.theme === 'dark') return 'dark';
    }

    return localStorage.getItem('dashboardTheme') === 'light' ? 'light' : 'dark';
  } catch (_error) {
    return 'dark';
  }
}

function shouldShowThemeDebugBadge() {
  try {
    var params = new URLSearchParams(window.location.search || '');
    if (params.get('themeDebug') === '1') {
      localStorage.setItem('hushThemeDebug', '1');
      return true;
    }
    if (params.get('themeDebug') === '0') {
      localStorage.removeItem('hushThemeDebug');
      return false;
    }
    return localStorage.getItem('hushThemeDebug') === '1';
  } catch (_error) {
    return false;
  }
}

function renderThemeDebugBadge(theme) {
  if (!document.body) return;

  var existing = document.getElementById('hushThemeDebugBadge');
  if (!shouldShowThemeDebugBadge()) {
  ...
  badge.textContent = 'Theme: ' + theme.toUpperCase() + ' | ' + page;
  if (theme === 'light') {
    badge.style.background = 'rgba(238,246,255,0.94)';
    badge.style.border = '1px solid rgba(98,132,173,0.5)';
    badge.style.color = '#17324d';
  } else {
    badge.style.background = 'rgba(13,27,45,0.9)';
    badge.style.border = '1px solid rgba(170,183,200,0.45)';
    badge.style.color = '#d6e0eb';
  }
}

function applySiteThemePreference(themeValue) {
  ...
  // Keep dashboard-specific class for compatibility with existing selectors.
  document.body.classList.toggle('dashboard-light-mode', isLight);
  document.documentElement.setAttribute('data-theme', theme);
  renderThemeDebugBadge(theme);
}

function initSiteThemePreference() {
  const applyNow = function () {
    applySiteThemePreference(resolveSiteThemePreference());
  };
  ...
  window.addEventListener('storage', function (event) {
    if (!event) return;
    if (event.key === 'dashboardTheme' || event.key === 'users' || event.key === 'lastSignedInUsername') {
      applySiteThemePreference(resolveSiteThemePreference());
    }
  });
}
...
try {
  window.applySiteThemePreference = applySiteThemePreference;
} catch (_error) {
  // ignore window assignment failures
}

initSiteThemePreference();

function getActiveThemeUsername() {
  try {
    return String(
      sessionStorage.getItem('username') ||
      localStorage.getItem('lastSignedInUsername') ||
...
function persistSiteThemePreference(themeValue) {
  var theme = themeValue === 'light' ? 'light' : 'dark';
  var username = getActiveThemeUsername();

  if (username && typeof setUserThemePreference === 'function') {
    try {
      setUserThemePreference(username, theme);
    } catch (_error) {
      // ignore user preference persistence failures
    }
  }
  ...
  try {
    ...
    // ignore dashboardTheme persistence failures
  }
}

function ensureGlobalThemeToggleStyles() {
  if (document.getElementById('hushGlobalThemeToggleStyles')) return;
  var style = document.createElement('style');
  style.id = 'hushGlobalThemeToggleStyles';
  style.textContent = [
    '#hushGlobalThemeToggle {',
    '  position: fixed;',
    '  right: 12px;',
    '  bottom: 12px;',
    '  z-index: 2147483645;',
    ...
    '  padding: 0 12px;',
    '  border-radius: 999px;',
    '  border: 1px solid rgba(170, 183, 200, 0.45);',
    ...
    '  box-shadow: 0 8px 20px rgba(3,5,9,0.35);',
    '}',
    'body.light-mode #hushGlobalThemeToggle,',
    'body.dashboard-light-mode #hushGlobalThemeToggle {',
    '  border-color: rgba(98,132,173,0.45);',
    '  background: rgba(255,252,247,0.94);',
    '  color: #17324d;',
    '  box-shadow: 0 8px 20px rgba(21,38,59,0.18);',
    '}',
    '#hushThemeDebugBadge + #hushGlobalThemeToggle {',
    '  bottom: 44px;',
    '}'
  ].join('\n');
...
function pageHasLocalThemeToggle() {
  return Boolean(
    document.getElementById('toggleDashboardTheme') ||
    document.getElementById('themeToggleBtn') ||
    document.getElementById('pwaThemeToggleBtn') ||
    document.querySelector('[data-action="theme"]')
  );
}

function updateGlobalThemeToggleLabel() {
  var button = document.getElementById('hushGlobalThemeToggle');
  if (!button) return;
  var currentTheme = resolveSiteThemePreference() === 'light' ? 'light' : 'dark';
  button.innerHTML = currentTheme === 'light' ? 'Mode: &#9728;' : 'Mode: &#127769;';
  button.setAttribute('aria-label', currentTheme === 'light' ? 'Switch to dark mode' : 'Switch to light mode');
  button.setAttribute('title', currentTheme === 'light' ? 'Switch to dark mode' : 'Switch to light mode');
  button.setAttribute('aria-pressed', currentTheme === 'light' ? 'true' : 'false');
}

function initGlobalThemeToggleFallback() {
  var mount = function () {
    if (!document.body) return;
    if (pageHasLocalThemeToggle()) return;

    ensureGlobalThemeToggleStyles();

    var button = document.getElementById('hushGlobalThemeToggle');
    if (!button) {
      button = document.createElement('button');
      button.id = 'hushGlobalThemeToggle';
      button.type = 'button';
      button.addEventListener('click', function () {
        ...
        try {
          ...
          // ignore page-level theme initializer errors
        }

        updateGlobalThemeToggleLabel();
      });
...
initGlobalThemeToggleFallback();

(function ensureSharedServiceWorkerRegistration() {
  if (!('serviceWorker' in navigator)) return;
  if (window.__hushSharedSwRegistered) return;
...
(function initPwaAppShell() {
  ...
  function getThemeToggleIconMarkup(theme) {
    return theme === 'light' ? 'Mode: &#9728;' : 'Mode: &#127769;';
  }

  function applyThemeToggleButtonState(button, theme) {
    if (!button) return;
    button.innerHTML = getThemeToggleIconMarkup(theme);
  ...
  function buildShell() {
    ...
    var activeTheme = applyPwaTheme();
    var themeIcon = getThemeToggleIconMarkup(activeTheme);
    ...
    sheet.innerHTML = [
      ...
      '  <button type="button" class="pwa-settings-action theme-icon-btn" data-action="theme" id="pwaThemeToggleBtn" aria-label="' + (activeTheme === 'light' ? 'Switch to dark mode' : 'Switch to light mode') + '" title="' + (activeTheme === 'light' ? 'Switch to dark mode' : 'Switch to light mode') + '" aria-pressed="' + (activeTheme === 'light' ? 'true' : 'false') + '">' + themeIcon + '</button>',
    ...
    sheet.addEventListener('click', function (event) {
      ...
      if (action === 'theme') {
        ...
        if (themeBtn) {
          applyThemeToggleButtonState(themeBtn, nextTheme);
        }
        return;
      }
      if (action === 'logout') {

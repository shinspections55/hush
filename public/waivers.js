document.addEventListener('DOMContentLoaded', () => {
    ...
    const waiverClockTeam = document.getElementById('waiverClockTeam');
    const waiverClockTimer = document.getElementById('waiverClockTimer');
    ...
    function updateWaiverClockStrip() {
        if (!waiverClockTeam || !waiverClockTimer) return;

        if (!waiverState) {
            waiverClockTeam.textContent = 'On the clock: --';
            waiverClockTimer.textContent = '--:--';
            return;
        }

        const currentTeam = getCurrentWaiverTeamName() || '--';

        if (!waiverState.active || waiverState.completed) {
            waiverClockTeam.textContent = 'Waivers complete';
            waiverClockTimer.textContent = '--:--';
            return;
        }
        ...
        if (isWaiverPrestartActive()) {
            ...
            waiverClockTeam.textContent = `Next: ${currentTeam}`;
            waiverClockTimer.textContent = prestartSeconds == null ? '--:--' : formatClock(prestartSeconds);
            return;
        }

        const secondsLeft = getWaiverSecondsRemaining();
        waiverClockTeam.textContent = `On the clock: ${currentTeam}`;
        waiverClockTimer.textContent = secondsLeft == null ? '--:--' : formatClock(secondsLeft);
    }

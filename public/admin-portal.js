document.addEventListener('DOMContentLoaded', () => {
  ...
  let cpuSilentProfiles = [];
  let cpuTiedProfiles = [];
  let cpuSilentBidRanges = {};
  let cpuTuningDefaults = {
    silent: {},
    tied: {},
    scenario: {
      profileIndex: 0,
  ...
  async function loadCpuReferenceData() {
    const payload = await requestJson('/api/admin/cpu-logic/reference');
    const reference = payload && payload.reference && typeof payload.reference === 'object' ? payload.reference : {};
    cpuSilentProfiles = Array.isArray(reference.silentProfiles) ? reference.silentProfiles : [];
    cpuTiedProfiles = Array.isArray(reference.tiedProfiles) ? reference.tiedProfiles : [];
    cpuSilentBidRanges = reference.silentBidRanges && typeof reference.silentBidRanges === 'object' ? reference.silentBidRanges : {};
    cpuTuningDefaults = mergeDeep(deepClone(cpuTuningDefaults), reference.defaults || {});
    builtinCpuModels = reference.builtinModels && typeof reference.builtinModels === 'object' ? reference.builtinModels : {};
  ...
  function ensureBuiltinCpuNamedModels() {
    ...
    Object.entries(builtinCpuModels).forEach(([name, configPatch]) => {
  ...
  async function ensureAdminSession() {
    if (!getAdminKey()) {
      throw new Error('Enter the admin key to unlock this page.');
    }
  }
  ...
  async function loadOverview() {
    ...
    try {
      const [traffic, system, delivery] = await Promise.all([
        requestJson('/api/admin/traffic'),
        requestJson('/api/admin/system-status'),
        requestJson('/api/admin/delivery/status')
      ]);
  ...
  async function initializePortalPage() {
    ...
    try {
      await loadCpuReferenceData();
      refreshCpuModelSelect('');
      renderCpuTuningLab();
      updateCpuLogicSourceMeta();
      await loadCpuLogicFromServer({ silent: true });
    } catch (error) {
      setCpuTuningStatus(error.message || 'Unable to load protected CPU reference data.', 'error');
    }

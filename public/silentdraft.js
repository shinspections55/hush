...
function initSilentDraft() {
    ...
    let draftReadinessComplete = false;
    let initialRoundWaitingForReadiness = false;
    ...
    function connectDraftSocketImmediately() {
        if (!window.io || !currentDraftCode || window.draftSocket) return;
        ...
        const announceReady = () => {
            ...
            window.draftSocket.emit('joinActiveDraft', currentDraftCode, username, { clientSentAt }, (response) => {
                if (response && response.ok) {
                    window.draftSocket.emit('draftReady', currentDraftCode, username, (readyResponse) => {
                        console.log('[silentdraft] Early member readiness:', {
                            draftCode: currentDraftCode,
                            username,
                            requiredMembers: readyResponse && readyResponse.requiredMembers,
                            complete: !!(readyResponse && readyResponse.complete),
                            timerReady: !!(readyResponse && readyResponse.timerReady)
                        });
                        if (readyResponse && readyResponse.complete) {
                            membersConfirmedForRound = true;
                            console.log('[silentdraft] Early readiness complete; starting deferred round preparation.');
                            startRoundPreparation();
                        }
                        if (readyResponse && readyResponse.timerReady && readyResponse.roundTimerScheduled) {
                            setDraftablePlayersVisible(true);
                            startLiveRoundTimer(
                                Number(readyResponse.roundTimerScheduled.startAt),
                                Number(readyResponse.roundTimerScheduled.endAt)
                            );
                        }
                    });
                    console.log('[silentdraft] Early draft socket ready:', currentDraftCode, username);
                }
            });
        };

        window.draftSocket.on('connect', announceReady);
        window.draftSocket.on('membersReadyForRound', () => {
            membersConfirmedForRound = true;
        });
        if (window.draftSocket.connected) announceReady();
    }

    connectDraftSocketImmediately();
    ...
    let draftBuildPending = false;
    let membersConfirmedForRound = false;
    ...
    let draftAppSectionLockApplied = false;
    let draftAppChromeObserved = false;
    let lastAppliedHeaderHeight = 0;
    let lastAppliedNavHeight = 0;
    ...
    const DRAFTROOM_DEFAULT_RANKINGS_CACHE_KEY = 'silentDraftDefaultRankingsCache';

    function buildDraftWhenPlayersReady() {
        if (!playerDataReady) {
            draftBuildPending = true;
            return;
        }
    ...
    async function loadDraftRoomDefaultRankings(forceRefresh = false) {
        ...
        try {
            ...
            if (applyDefaultRankingsPayload(data)) {
                try {
                    localStorage.setItem(DRAFTROOM_DEFAULT_RANKINGS_CACHE_KEY, JSON.stringify(data));
                } catch (_error) {
                    // Cache is optional; the live data remains authoritative.
                }
        ...
        // Cache is only a fallback so stale players never override live Admin rankings.
        if (draftRoomDefaultRankings.length === 0) {
            try {
                const cached = JSON.parse(localStorage.getItem(DRAFTROOM_DEFAULT_RANKINGS_CACHE_KEY) || 'null');
                if (Array.isArray(cached) && applyDefaultRankingsPayload(cached)) {
                    console.warn(`[silentdraft] API unavailable; using ${draftRoomDefaultRankings.length} cached default rankings`);
                }
            } catch (_error) {
    ...
    if (!window.draftSocket) window.draftSocket = null;
    ...
    if (window.draftSocket && currentDraftCode) {
        ...
        const updateLastSeenDraftStateVersion = (nextVersion) => {
            const parsedVersion = Number(nextVersion);
            if (Number.isFinite(parsedVersion) && parsedVersion > lastSeenDraftStateVersion) {
                lastSeenDraftStateVersion = parsedVersion;
            }
        };

        const applyDraftMetaFromServer = (draftMeta) => {
            if (!draftMeta) return;
            draftHostName = draftMeta.host || (Array.isArray(draftMeta.members) ? draftMeta.members[0] : draftHostName);
            allDraftMembers = Array.isArray(draftMeta.members) ? draftMeta.members.slice() : allDraftMembers;
            ...
            if (window.isHost !== hostFromState) {
                ...
                updatePauseButtonVisibility();
                console.log('[silentdraft] Host role updated from server state:', window.isHost);
            }
        };

        function sameRoundPlayers(prevPlayers, nextPlayers) {
            ...
            for (let i = 0; i < next.length; i++) {
                ...
                if (Number(a.bid || 0) !== Number(b.bid || 0)) return false;
            }
            return true;
        }
        ...
        const applyRoundPlayersPayload = (payload, sourceLabel = 'roundPlayersSet') => {
            const roundPlayers = Array.isArray(payload)
                ? payload
            ...
            if (!window.isHost && shouldRenderPlayers) {
                displayRoundPlayers(roundPlayers);
            }
            if (window.draftSocket && currentDraftCode && payload && payload.roundNumber) {
                window.draftSocket.emit('roundPlayersReady', currentDraftCode, username, payload.roundNumber);
            }
        };

        const applyAutoDraftStatusPayload = (payload, sourceLabel = 'autoDraftStatusChanged') => {
            updateLastSeenDraftStateVersion(payload && payload.stateVersion);
            const nextStatusMap = (payload && payload.statuses) ? payload.statuses : (payload || {});
            const hadChange = JSON.stringify(autoDraftStatusByTeam || {}) !== JSON.stringify(nextStatusMap || {});
            autoDraftStatusByTeam = nextStatusMap || {};
            ...
            console.log('[silentdraft] Auto-draft status synced from', sourceLabel, 'changed=', hadChange);
        };

        const applyRosterSettingsUpdatedPayload = (data, sourceLabel = 'rosterSettingsUpdated') => {
            if (!data) return;
            updateLastSeenDraftStateVersion(data.stateVersion);
            ...
            if (typeof data.roundTimerMinutes !== 'undefined') {
                console.log('[silentdraft] rosterSettingsUpdated applying roundTimerMinutes:', data.roundTimerMinutes, 'source=', sourceLabel);
        ...
        const applyDraftChatMessagePayload = (payload) => {
            if (!payload || typeof payload.text !== 'string') return;
            updateLastSeenDraftStateVersion(payload.stateVersion);
        ...
        const handleLiveAuctionSyncPayload = (auctionState, sourceLabel = 'liveAuctionSync') => {
            if (!auctionState || !auctionState.auctionId || !auctionState.playerId) {
                return;
            }

            updateLastSeenDraftStateVersion(auctionState.stateVersion);
            ...
            console.warn('[silentdraft] Recovering active live auction from sync payload:', auctionState.auctionId, 'source=', sourceLabel);
            try {
        ...
        const applyRoundStartedPayload = (draftState, sourceLabel = 'roundStarted') => {
            updateLastSeenDraftStateVersion(draftState && draftState.stateVersion);
            ...
            if (lastServerRoundStarted === nextRound) {
                console.log('[silentdraft] Ignoring duplicate roundStarted for round', nextRound, 'source=', sourceLabel);
                return;
            }

            console.log('[silentdraft] New round started from server:', nextRound, 'source=', sourceLabel);
            lastServerRoundStarted = nextRound;
            currentRound = nextRound;
            window.syncedRoundPlayers = null;
        ...
        const applyDraftStatePayload = (draftState, sourceLabel = 'draftStateSync') => {
            if (!draftState) return;
            updateLastSeenDraftStateVersion(draftState.stateVersion);
            console.log('[silentdraft] Draft state synced:', sourceLabel, draftState);
            logActiveCpuLogicPreset();
            const serverRound = Number(draftState.currentRound);
            ...
            if (draftState && typeof draftState.roundTimerMinutes !== 'undefined') {
                console.log('[silentdraft] draftStateSync applying roundTimerMinutes:', draftState.roundTimerMinutes, 'source=', sourceLabel);
                applyRoundTimerMinutes(draftState.roundTimerMinutes);
            }
            autoDraftEnabled = !!autoDraftStatusByTeam[username];
            updateAutoDraftToggleUI();
            if (draftState.currentPlayers && draftState.currentPlayers.length > 0) {
                const shouldRenderPlayers = !sameRoundPlayers(window.syncedRoundPlayers, draftState.currentPlayers);
                window.syncedRoundPlayers = draftState.currentPlayers;
                if (!window.isHost && shouldRenderPlayers) {
                    displayRoundPlayers(draftState.currentPlayers);
                }
                if (window.draftSocket && currentDraftCode && Number.isFinite(Number(draftState.currentRound))) {
                    window.draftSocket.emit('roundPlayersReady', currentDraftCode, username, draftState.currentRound);
                }
            }
            if (draftState.roundStartAt && draftState.roundEndsAt) {
                startLiveRoundTimer(draftState.roundStartAt, draftState.roundEndsAt);
            }
        };

        const applyResumeEvent = (event) => {
            if (!event || !event.type) return;
            switch (event.type) {
                case 'rosterSettingsUpdated':
                    applyRosterSettingsUpdatedPayload(event.payload, 'resume-event');
                    break;
                case 'autoDraftStatusChanged':
                    applyAutoDraftStatusPayload(event.payload, 'resume-event');
                    break;
                case 'draftChatMessage':
                    applyDraftChatMessagePayload(event.payload);
                    break;
                case 'roundPlayersSet':
                    applyRoundPlayersPayload(event.payload, 'resume-event');
                    break;
                case 'roundResults':
                    handleAuthoritativeRoundResults(event.payload, 'resume-event');
                    break;
                case 'roundStarted':
                    applyRoundStartedPayload(event.payload, 'resume-event');
                    break;
                default:
                    updateLastSeenDraftStateVersion(event.version);
                    break;
            }
        };
        ...
        const syncDraftSocketRooms = () => {
            ...
            const clientSentAt = Date.now();
            return emitSocketAckWithRetry('joinActiveDraft', [currentDraftCode, username, { clientSentAt }], {
                timeoutMs: 7000,
                ...
                if (response && response.ok) {
                    ...
                    const acknowledgedClientSentAt = Number(response.clientSentAt || 0);
                    if (Number.isFinite(Number(response.serverTs)) && acknowledgedClientSentAt > 0) {
                        serverClockOffsetMs = Number(response.serverTs) - ((acknowledgedClientSentAt + clientReceivedAt) / 2);
                    }
                    updateLastSeenDraftStateVersion(response.stateVersion);
                    ...
                    if (response.roundTimerScheduled && Number(response.roundTimerScheduled.currentRound) === currentRound) {
                        ...
                        setDraftablePlayersVisible(true);
                        startLiveRoundTimer(
                            Number(response.roundTimerScheduled.startAt),
                            Number(response.roundTimerScheduled.endAt)
                        );
                    }
                    return true;
                }

                console.warn('[silentdraft] joinActiveDraft rejected:', response);
                return false;
            }).catch((error) => {
                console.warn('[silentdraft] Failed to sync active draft room:', error);
                return false;
            });
        };

        const requestFreshDraftState = () => {
            if (!(window.draftSocket && currentDraftCode)) return Promise.resolve(false);
            return new Promise((resolve) => {
                window.draftSocket.emit('getDraftState', currentDraftCode, (response) => {
                    if (response && response.ok && response.draft) {
                        const draft = response.draft;
                        applyDraftMetaFromServer(draft);
                        applyDraftStatePayload(draft.draftState || {}, 'fresh-state');

                        const currentPlayers = Array.isArray(draft.draftState && draft.draftState.currentPlayers)
                            ? draft.draftState.currentPlayers
                            : (Array.isArray(draft.currentPlayers) ? draft.currentPlayers : []);

                        if (currentPlayers.length > 0) {
                            const shouldRenderPlayers = !sameRoundPlayers(window.syncedRoundPlayers, currentPlayers);
                            window.syncedRoundPlayers = currentPlayers;
                            if (!window.isHost && shouldRenderPlayers) {
                                displayRoundPlayers(currentPlayers);
                            }
                            window.draftSocket.emit('roundPlayersReady', currentDraftCode, username, draft.draftState.currentRound);
                            console.log('[silentdraft] Fresh state restored current round players:', currentPlayers.length, 'rendered:', shouldRenderPlayers);
                        } else {
        ...
        const resumeActiveDraftState = (reason = 'unknown') => {
            if (!(window.draftSocket && currentDraftCode)) {
                return Promise.resolve(false);
            }

            return emitSocketAckWithRetry('resumeActiveDraft', [currentDraftCode, username, lastSeenDraftStateVersion], {
                timeoutMs: 7000,
                overallTimeoutMs: 12000,
                maxRetries: 1,
                suppressRetryWarning: true
                ...
                if (!(response && response.ok)) {
                    console.warn('[silentdraft] resumeActiveDraft rejected:', response);
                    return false;
                }

                updateLastSeenDraftStateVersion(response.currentVersion);
                applyDraftMetaFromServer(response.draftMeta);

                if (response.mode === 'full' && response.draft) {
                    applyDraftMetaFromServer(response.draft);
                    applyDraftStatePayload(response.draft.draftState || {}, `resume-full:${reason}`);
                } else if (response.mode === 'events' && Array.isArray(response.events)) {
                    response.events.forEach(applyResumeEvent);
                }

                if (response.pendingRoundResults) {
                    handleAuthoritativeRoundResults(response.pendingRoundResults, `resume-round-results:${reason}`);
                }

                if (response.activeAuction) {
                    handleLiveAuctionSyncPayload(response.activeAuction, `resume-live-auction:${reason}`);
                }

                console.log('[silentdraft] Resume state applied:', response.mode, 'version=', response.currentVersion, 'reason=', reason);
                return true;
            }).catch((error) => {
                console.warn('[silentdraft] resumeActiveDraft failed:', error);
                return false;
            });
        };

        const resyncDraftConnection = (reason = 'unknown') => {
            return syncDraftSocketRooms().then((joined) => {
                if (!joined) {
                    return requestFreshDraftState();
                }

                return resumeActiveDraftState(reason).then((resumed) => {
                    if (resumed) {
                        return true;
                    }
                    return requestFreshDraftState();
        ...
        const runDraftHeartbeat = () => {
            if (!(window.draftSocket && currentDraftCode)) return;
            if (!window.draftSocket.connected) return;
            ...
            const timeout = setTimeout(() => {
                if (settled) return;
                settled = true;
                console.warn('[silentdraft] Application heartbeat timed out; reconnecting draft socket');
                scheduleConnectionResync('heartbeat-timeout', 0);
                try {
                    if (window.draftSocket.connected) window.draftSocket.disconnect();
                    window.draftSocket.connect();
                } catch (error) {
                    console.warn('[silentdraft] Heartbeat reconnect failed:', error);
                }
            }, 5000);

            window.draftSocket.emit('hushHeartbeat', { clientTs }, (response) => {
                if (settled) return;
                settled = true;
                clearTimeout(timeout);
                if (!response || response.ok === false) {
                    console.warn('[silentdraft] Application heartbeat rejected:', response);
                    scheduleConnectionResync('heartbeat-rejected', 0);
                    return;
                }
                const serverTs = Number(response.serverTs);
                if (Number.isFinite(serverTs)) {
                    serverClockOffsetMs = serverTs - ((clientTs + Date.now()) / 2);
                }
            });
        };
        ...
        draftHeartbeatTimer = setInterval(runDraftHeartbeat, 20000);
        ...
        window.draftSocket.on('connect', () => {
            resyncDraftConnection('socket-connect');
            updateSocketConnectionIndicator(true);
        ...
        window.draftSocket.on('roundDiagnostics', (payload) => {
            if (window.__HUSH_DEBUG_ROUND === true) {
        ...
        window.draftSocket.on('liveAuctionSync', (auctionState) => {
            handleLiveAuctionSyncPayload(auctionState, 'liveAuctionSync');
        });
        ...
        window.draftSocket.on('roundPlayersSet', (payload) => {
            applyRoundPlayersPayload(payload, 'roundPlayersSet');
        });
        ...
        window.draftSocket.on('roundStarted', (draftState) => {
            applyRoundStartedPayload(draftState, 'roundStarted');
        });

        window.draftSocket.on('draftReadiness', (readiness) => {
            draftReadinessComplete = !!(readiness && readiness.complete);
            if (draftReadinessComplete && window.isHost && initialRoundWaitingForReadiness) {
                initialRoundWaitingForReadiness = false;
                startRound();
            }
        });

        window.draftSocket.on('membersReadyForRound', (payload) => {
            membersConfirmedForRound = true;
            console.log('[silentdraft] All lobby members confirmed; allowing rankings, AV, and player preparation:', payload);
            startRoundPreparation();
        });
        ...
        window.draftSocket.on('readinessDebug', (readiness) => {
            console.warn('[silentdraft] Readiness debug:', readiness);
            if (Array.isArray(readiness && readiness.missingMembers)) {
                latestMemberReadiness.missingMembers = readiness.missingMembers;
            }
        });

        window.draftSocket.on('readinessReport', (report) => {
            ...
            if (report && report.timerScheduled && report.roundStartAt && report.roundEndsAt
                ...
                clearRoundSyncCountdown();
                setDraftablePlayersVisible(true);
                startLiveRoundTimer(Number(report.roundStartAt), Number(report.roundEndsAt));
            }
        });

        window.draftSocket.on('draftStateSync', (draftState) => {
            applyDraftStatePayload(draftState, 'draftStateSync');
        });

        window.draftSocket.on('autoDraftStatusSync', (statusMap) => {
            applyAutoDraftStatusPayload({ statuses: statusMap || {} }, 'autoDraftStatusSync');
        });

        window.draftSocket.on('autoDraftStatusChanged', (payload) => {
            applyAutoDraftStatusPayload(payload, 'autoDraftStatusChanged');
        });

        window.draftSocket.on('rosterSettingsUpdated', (data) => {
            applyRosterSettingsUpdatedPayload(data, 'rosterSettingsUpdated');
        });

        window.draftSocket.on('draftChatMessage', (payload) => {
            applyDraftChatMessagePayload(payload);
        });
    }
    ...
    window.scheduleCurrentRoundBidSync = scheduleCurrentRoundBidSync;

    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') {
            scheduleCurrentRoundBidSync(0);
        }
    });
  ...
  // Full player list (top 250 PPR players)
    ...
    function setupRightViewTabs() {
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                applyRightViewMode();
                // Keep the PWA bottom nav highlight in sync with the right-hand tabs.
                if (draftAppSectionNavEnabled) {
                    applyDraftAppSectionMode(draftRoomRightViewMode, { persist: true });
                }
            });
        });
    }
    ...
    function applyDraftAppStabilityLock() {
        // Only force the Players default on first paint; later viewport changes
        // (mobile keyboard, rotation) must not clobber the user's chosen section.
        if (PWA_LAYOUT_STABILITY_LOCK.forcePlayersDefaultSection && !draftAppSectionLockApplied) {
            draftAppSectionLockApplied = true;
            draftAppSectionViewMode = 'players';
        }
    }
    ...
    function syncDraftAppChromeHeights() {
        ...
        // iOS reports shifted rects for fixed chrome while the keyboard is open.
        ...
        if (headerBar) {
            const measuredHeaderHeight = Math.round(headerBar.getBoundingClientRect().height || 0);
            if (measuredHeaderHeight > 0) {
                const stableHeaderHeight = Math.max(56, Math.min(120, measuredHeaderHeight));
                if (stableHeaderHeight !== lastAppliedHeaderHeight) {
                    lastAppliedHeaderHeight = stableHeaderHeight;
                    document.body.style.setProperty('--draft-app-header-height', `${stableHeaderHeight}px`);
                }
            }
        }
        if (appNav) {
            const measuredNavHeight = Math.round(appNav.getBoundingClientRect().height || 0);
            if (measuredNavHeight > 0) {
                const stableNavHeight = Math.max(56, Math.min(98, measuredNavHeight));
                if (stableNavHeight !== lastAppliedNavHeight) {
                    lastAppliedNavHeight = stableNavHeight;
    ...
    function observeDraftAppChrome() {
        if (draftAppChromeObserved) return;
        draftAppChromeObserved = true;

        const headerBar = document.querySelector('.header-bar');
        const appNav = document.getElementById('silentdraft-app-nav');

        if (typeof ResizeObserver === 'function') {
            const observer = new ResizeObserver(() => syncDraftAppChromeHeights());
            if (headerBar) observer.observe(headerBar);
            if (appNav) observer.observe(appNav);
        }

        // Fonts and the second header row settle after first paint.
        [0, 120, 400, 1000].forEach((delay) => {
            setTimeout(() => syncDraftAppChromeHeights(), delay);
        });
        if (document.fonts && typeof document.fonts.ready?.then === 'function') {
            document.fonts.ready.then(() => syncDraftAppChromeHeights()).catch(() => {});
        }
    }
    ...
    function applyDraftAppSectionMode(section, options = {}) {
        ...
        if (document.body && (draftAppSectionNavEnabled || document.body.hasAttribute('data-draft-app-section'))) {
    ...
    function setupDraftChat() {
        ...
        if (gifAddButton) {
            ...
            gifAddButton.innerHTML = '<img class="gif-btn-brand-mark" src="/Poweredby_100px-White_VertLogo.png" alt="Powered by GIPHY" loading="lazy" decoding="async">';
        }
    ...
    function setupVoiceLobby() {
        if (!(window.io && currentDraftCode)) return;
        if (window.__hushVoiceLobbyInitialized) return;

        const listEl = document.getElementById('draft-voice-list');
        const statusEl = document.getElementById('draft-voice-status');
        const joinBtn = document.getElementById('draft-voice-join');
        const muteBtn = document.getElementById('draft-voice-mute');
        const deafenBtn = document.getElementById('draft-voice-deafen');
        const pttBtn = document.getElementById('draft-voice-ptt');
        const leaveBtn = document.getElementById('draft-voice-leave');
        const audioHost = document.getElementById('draft-voice-audio-host');
        const socket = window.draftSocket;

        if (!listEl || !statusEl || !joinBtn || !muteBtn || !deafenBtn || !pttBtn || !leaveBtn || !audioHost || !socket) {
            return;
        }

        window.__hushVoiceLobbyInitialized = true;
        ...
        function participantSort(a, b) {
            const aSelf = a.socketId === socket.id ? 0 : 1;
            const bSelf = b.socketId === socket.id ? 0 : 1;
        ...
        function updateVoiceButtons() {
            ...
            leaveBtn.disabled = !localVoiceJoined;
        ...
        function emitVoiceJoinWithTimeout(timeoutMs = 15000) {
            return new Promise((resolve, reject) => {
                ...
                socket.emit('voice:join', currentDraftCode, payload, (response) => {
                    if (settled) return;
                    settled = true;
                    window.clearTimeout(timerId);
        ...
        function renderVoiceParticipants() {
            ...
            const hasSelf = list.some((entry) => entry && entry.socketId === socket.id);
            if (!hasSelf) {
                list.push({
                    socketId: socket.id,
                    username,
            ...
            if (!list.length) {
                listEl.innerHTML = '<div class="draft-chat-empty">No one in voice yet.</div>';
                return;
            }
            ...
            listEl.innerHTML = list.sort(participantSort).map((entry) => {
                ...
                const selfTag = entry.socketId === socket.id ? ' (You)' : '';
                const isSelf = entry.socketId === socket.id;
                const volumePercent = getRemoteVolumePercent(entry.socketId);
                const inlineAction = isSelf
                    ? (localVoiceJoined
                        ? '<button type="button" class="draft-voice-inline-btn is-leave" data-voice-inline="leave" aria-label="Leave voice" title="Leave voice"><span class="draft-voice-inline-mic">🎤</span><span class="draft-voice-inline-x">✕</span></button>'
                        : '<button type="button" class="draft-voice-inline-btn" data-voice-inline="join" aria-label="Join voice" title="Join voice">Join <span class="draft-voice-inline-mic">🎤</span></button>')
                    : `<span class="draft-voice-member-state">${stateText}</span><div class="draft-voice-volume-wrap"><input type="range" class="draft-voice-volume" data-voice-volume-socket="${String(entry.socketId || '')}" min="0" max="200" step="1" value="${volumePercent}" aria-label="${safeName} volume"><span class="draft-voice-volume-value" data-voice-volume-value="${String(entry.socketId || '')}">${volumePercent}%</span></div>`;

                return `<div class="draft-voice-member${speakingClass}"><span class="draft-voice-member-name"><span class="draft-voice-inline-mic">🎤</span>${safeName}${selfTag}</span><span class="draft-voice-self-action">${inlineAction}</span></div>`;
            }).join('');
        }
        ...
        function emitVoiceMuteState() {
            ...
            socket.emit('voice:mute-state', currentDraftCode, {
                muted: selfMuted,
                deafened: selfDeafened
            });
            const selfParticipant = participantsBySocketId.get(socket.id);
            if (selfParticipant) {
                selfParticipant.muted = selfMuted;
                selfParticipant.deafened = selfDeafened;
                selfParticipant.speaking = false;
                participantsBySocketId.set(socket.id, selfParticipant);
        ...
        function stopSpeakingLoop() {
            ...
            if (localSpeaking) {
                localSpeaking = false;
                socket.emit('voice:speaking', currentDraftCode, false);
            }
        }
        ...
        function startSpeakingLoop() {
            ...
            speakingLoopId = window.setInterval(() => {
                ...
                if (isSpeakingNow !== localSpeaking) {
                    ...
                    socket.emit('voice:speaking', currentDraftCode, localSpeaking);
                }
            }, 220);
        }
        ...
        function getOrCreatePeerConnection(remoteSocketId) {
            if (!remoteSocketId || remoteSocketId === socket.id) return null;
            ...
            pc.onicecandidate = (event) => {
                if (event.candidate) {
                    socket.emit('voice:signal-ice', currentDraftCode, remoteSocketId, event.candidate);
                }
            };
        ...
        async function createOfferFor(remoteSocketId) {
            ...
            socket.emit('voice:signal-offer', currentDraftCode, remoteSocketId, offer);
        }
        ...
        async function finalizeVoiceJoin(participantsRaw) {
            ...
            for (const remoteSocketId of existingPeers) {
                try {
                    ...
                    console.warn('[voice] Failed to create offer for peer:', remoteSocketId, error);
                }
            }
        }
        ...
        async function joinVoiceLobby() {
            ...
            try {
                ...
                if (!localVoiceJoined) {
                    console.warn('[voice] Join ack timeout/error:', error);
                }
            }
            ...
            if (!response || !response.ok) {
                ...
                showNotification(getVoiceJoinErrorMessage(response));
                if (localStream) {
        ...
        function leaveVoiceLobby({ notifyServer = true } = {}) {
            if (notifyServer) {
                socket.emit('voice:leave', currentDraftCode, () => {});
            }
        ...
        listEl.addEventListener('click', (event) => {
            ...
            if (action === 'join') {
                joinVoiceLobby().catch((error) => {
                    console.warn('[voice] Inline join failed:', error);
                });
                return;
            }
            if (action === 'leave') {
        ...
        socket.on('voice:participants', (participants) => {
            ...
            const hasSelf = entries.some((entry) => String(entry && entry.socketId || '').trim() === String(socket.id || '').trim());

            if (!localVoiceJoined && !(joinInFlight && hasSelf)) return;

            if (joinInFlight && hasSelf) {
                finalizeVoiceJoin(entries).catch((error) => {
                    console.warn('[voice] finalize from participants failed:', error);
                });
                return;
            }
        ...
        socket.on('voice:peer-joined', (entry) => {
        ...
        socket.on('voice:peer-left', ({ socketId: remoteSocketId }) => {
        ...
        socket.on('voice:peer-updated', (entry) => {
            if (!entry || !entry.socketId) return;
        ...
        socket.on('voice:signal-offer', async ({ fromSocketId, sdp }) => {
            ...
            try {
                ...
                socket.emit('voice:signal-answer', currentDraftCode, String(fromSocketId), answer);
            } catch (error) {
                console.warn('[voice] Failed to handle offer:', error);
            }
        });

        socket.on('voice:signal-answer', async ({ fromSocketId, sdp }) => {
            if (!localVoiceJoined || !fromSocketId || !sdp) return;
            ...
            try {
                ...
                console.warn('[voice] Failed to handle answer:', error);
            }
        });

        socket.on('voice:signal-ice', async ({ fromSocketId, candidate }) => {
            ...
            try {
                ...
                console.warn('[voice] Failed to add ICE candidate:', error);
            }
        });
        ...
        socket.on('connect', () => {
            if (!localVoiceJoined || !localStream) return;
            socket.emit('voice:join', currentDraftCode, {
                muted: selfMuted,
                deafened: selfDeafened
            }, async (response) => {
                ...
                for (const remoteSocketId of existingPeers) {
                    try {
                        ...
                        console.warn('[voice] Reconnect offer failed:', error);
                    }
                }
            });
        });
    ...
    function updateUI(roundPlayers) {
        ...
        if (playerList) {
            ...
            const queueBidSync = (delayMs) => {
                const scheduler = window.scheduleCurrentRoundBidSync;
                if (typeof scheduler === 'function') scheduler(delayMs);
            };
            document.querySelectorAll('input[data-player-id]').forEach(input => {
...
const otherTeams = teams.filter(t => t.name !== username && isValidRosterAddition(t, player));
    ...
    function setDraftablePlayersVisible(visible) {
        const playerList = document.getElementById('players-list');
        if (!playerList) return;
        playerList.hidden = !visible;
        playerList.setAttribute('aria-hidden', visible ? 'false' : 'true');
    }

    function startLiveRoundTimer(startAt, endAt) {
        const timerElement = document.getElementById('timer');
        setDraftablePlayersVisible(true);
        setRoundSubmitEnabled(false, 'Waiting for synchronized start');
        ...
        const scheduledStartAt = Number(startAt) - serverClockOffsetMs;
        const scheduledEndAt = Number(endAt) - serverClockOffsetMs;
        if (!Number.isFinite(scheduledStartAt) || !Number.isFinite(scheduledEndAt)) {
            console.warn('[silentdraft] Missing authoritative round timer schedule');
            return;
        }

        const renderAuthoritativeTimer = () => {
            const remainingMs = scheduledEndAt - Date.now();
            timer = Math.max(0, Math.ceil(remainingMs / 1000));
            if (timerElement) {
            ...
            if (Date.now() >= scheduledEndAt) {
                clearInterval(timerInterval);
                timerInterval = null;
                console.log('[silentdraft] Authoritative timer expired, processing round');
                window.handleRoundTimerExpired();
            }
        };

        const waitForScheduledStart = () => {
            ...
            clearInterval(timerInterval);
            timerInterval = setInterval(renderAuthoritativeTimer, 250);
            renderAuthoritativeTimer();
        };

        timerInterval = setInterval(waitForScheduledStart, 100);
        ...
        if (window.isHost) {
            ...
            if (window.draftSocket && currentDraftCode) {
                window.draftSocket.emit('setRoundPlayers', currentDraftCode, roundPlayers, (response) => {
                    if (response && response.ok) {
                        window.draftSocket.emit('roundPlayersReady', currentDraftCode, username, currentRound);
        ...
        if (upBtn && userInTie) {
            ...
            const handleBidUp = () => {
                ...
                if (backedOut) {
                    ...
                    showNotification('You have backed out of this auction');
                    return;
                }
                ...
                if (!yourTeam) {
                    showNotification('Unable to resolve your team. Please refresh and rejoin the draft.');
                    return;
                }
                
                if (newBid > yourTeam.budget) {
                    console.log('[upBtn] Bid exceeds budget, rejecting');
                    showNotification(`Bid exceeds your budget of $${yourTeam.budget}`);
                    return;
                }
                
                if (newBid > 999) {
                    console.log('[upBtn] Bid exceeds max (999), rejecting');
                    showNotification('Maximum bid is $999');
                    return;
                }
                ...
                window.draftSocket.emit('placeLiveAuctionBid', currentDraftCode, auctionId, newBid, (response) => {
                    ...
                    if (!response || !response.ok) {
                        ...
                        showNotification('Failed to place bid: ' + (response?.reason || 'unknown error'));
                    } else {
...
function showRoundResultsModal(serverResults, roundPlayers, onComplete, meta = {}) {
    ...
    function startRoundPreparation() {
        if (roundPreparationStarted) return;
        roundPreparationStarted = true;
        loadPlayers().then(() => {
        playerDataReady = true;
        if (draftBuildPending) {
            draftBuildPending = false;
            buildDraftWhenPlayersReady();
        }

        // Refresh secondary ranking views without delaying the draftable player list.
        return Promise.all([
    ...
    // Start Socket.IO immediately; rankings, AV, and player work follows.
    initializeDraft();
    startRoundPreparation();
}
